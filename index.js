...
//Creating end point for new collection of data
...
//Creating end point for popular in new section

app.get('/popularinwomen',async(req,res)=>{
    let allProducts = await Product.find({category: 'women'})
    let pupularInWomen = allProducts.slice(0,4)
    console.log("Popular in women Collection Fetched")
    res.send(pupularInWomen)
})
...
const fetchUser = async(req,res,next)=>{
    ...
    if(!token){
        res.status(401).send({errors:"Authentication token not found!, Please login"})
    }
    else{
        try{
...
//Creating end point to add products in cart data
...
app.post('/addtocart',fetchUser,async (req,res)=>{
    ...
    console.log(`Item ${req.body.itemId} added to cart successfully`)
...
app.post('/removefromcart',fetchUser,async (req,res)=>{
    ...
    if (userData.cartData[req.body.itemId]>0)
    {
        ...
        console.log(`Item ${req.body.itemId} removed from cart successfully`)
    }   
})

// Creating end point to retain and restore users cart data

app.post('/getcart',fetchUser,async(req,res)=>{
     console.log('getCart fetched')
     let userData = await Users.findOne({_id:req.user.id});
